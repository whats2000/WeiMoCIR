...
class BLIPTextEncoder(torch.nn.Module):
    def __init__(self, pretrained_weight_path, med_config_path, use_pretrained_proj_layer=True, vit='base'):
        """
        ...
        encoder_config.encoder_width = 768 if vit=='base' else 1024
        ...
        if use_pretrained_proj_layer:
            ...
            self.text_proj = nn.Linear(768,
                                       640)  # assuming using clip img encoder, so get 640 out here
    ...
    def forward(self, caption, max_length, device):
        ...
        caption_embedding = self.text_encoder(caption.input_ids, attention_mask=caption.attention_mask,
