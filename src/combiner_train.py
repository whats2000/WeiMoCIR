...
def combiner_training_fiq(train_dress_types: List[str], val_dress_types: List[str],
                            ...
                            projection_dim: int, hidden_dim: int, num_epochs: int,
                            combiner_lr: float, batch_size: int, blip_bs: int, validation_frequency: int,
                            transform: str, input_dim: int, feature_dim: int,
                            save_training: bool, save_best: bool, **kwargs):
    """
    Train the Combiner on FashionIQ dataset keeping frozed the BLIP model
    ...
    :param blip_bs: batch size of the BLIP feature extraction
    ...
    print("blip text encoder created.")
    blip_text_encoder = blip_text_encoder.eval()
    ...
    print("blip img encoder created.")
    blip_img_encoder = blip_img_encoder.eval()

    if transform == "squarepad":
        preprocess = squarepad_transform(input_dim)
        print('Square pad preprocess pipeline is used')
        warnings.warn(f"transform type {transform} may not yield the best performance")
    ...
    for epoch in range(num_epochs):
        ...
        with experiment.train():
            ...
            for idx, (reference_images, target_images, captions, negated_captions) in enumerate(train_bar):  # Load a batch of triplets
                ...
                negated_flattened_captions: list = np.array(negated_captions).T.flatten().tolist()
                input_captions = generate_randomized_fiq_caption(flattened_captions)
                negated_input_captions = generate_randomized_fiq_caption(negated_flattened_captions)

                # Extract the features with BLIP
                with torch.no_grad():
                    reference_images_list = torch.split(reference_images, blip_bs)
                    reference_image_features = torch.vstack(
                        [blip_img_encoder(mini_batch).float() for mini_batch in reference_images_list])
                    target_images_list = torch.split(target_images, blip_bs)
                    target_image_features = torch.vstack(
                        [blip_img_encoder(mini_batch).float() for mini_batch in target_images_list])

                    text_features = blip_text_encoder(input_captions, max_length=77, device=device).float()
                    negated_text_features = blip_text_encoder(negated_input_captions, max_length=77, device=device).float()
                ...
                with torch.cuda.amp.autocast():
                    ...
                    Tij = target_image_features.unsqueeze(1).repeat(1, images_in_batch, 1).transpose(0,1)
                    ti = negated_text_features.unsqueeze(1).repeat(1, images_in_batch, 1)
                    # below is just the combiner.forward() with some modifications
                    # operations done to Ri should be before the dimension changes
                    reference_image_features = combiner.project_targets(reference_image_features)
                    reference_image_features = torch.nn.functional.normalize(reference_image_features, dim=-1)
                    Ri = reference_image_features.unsqueeze(1).repeat(1, images_in_batch, 1).unsqueeze(-1)
                    Pi = combiner.combine_features(Tij, ti).unsqueeze(-2)

                    logits_r = combiner.logit_scale * Pi @ Ri
                    logits_r = logits_r.squeeze()

                    loss_r = crossentropy_criterion(logits_r, ground_truth)
                    loss = loss_f + .5 * loss_r
        ...
        if epoch % validation_frequency == 0:
            ...
            # Save model
            if save_training:
                save_model('combiner_last', epoch, combiner, training_path, optimizer)
                if save_best and results_dict['average_recall'] > best_avg_recall:
                    best_avg_recall = results_dict['average_recall']
                    save_model('combiner', epoch, combiner, training_path, optimizer)
                elif not save_best:
                    save_model(f'combiner_{epoch}', epoch, combiner, training_path, optimizer)


def combiner_training_cirr(projection_dim: int, hidden_dim: int, num_epochs: int,
                           num_workers: int,
                           blip_pretrained_path: str, med_config_path: str,
                           combiner_lr: float, batch_size: int, blip_bs: int, validation_frequency: int, 
                           transform: str, input_dim: int, feature_dim: int,
                           save_training: bool, save_best: bool, **kwargs):
    """
    Train the Combiner on CIRR dataset keeping frozen the BLIP model
    :param projection_dim: Combiner projection dimension
    :param hidden_dim: Combiner hidden dimension
    :param num_epochs: number of epochs
    :param combiner_lr: Combiner learning rate
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        base_path / f"models/combiner_trained_on_cirr_BLIPImgTxt_{training_start}")
    training_path.mkdir(exist_ok=False, parents=True)
    print(f"training start time {training_start}")
    print(f"local folder {training_path}")
    ...
    if kwargs.get("blip_model_path"):
        print('Trying to load the fine-tuned BLIP text encoder')
        blip_model_path = kwargs["blip_model_path"]
        state_dict = torch.load(blip_model_path, map_location=device)
        blip_text_encoder.load_state_dict(state_dict["BLIPTextEncoder"])
        print('BLIP text encoder loaded successfully')
        ...
        with experiment.train():
            ...
            for idx, (reference_images, target_images, captions, negated_captions) in enumerate(train_bar):  # Load a batch of triplets
                ...
                with torch.no_grad():
                    ...
                    text_features = blip_text_encoder(captions, max_length=77, device=device).float()
                    negated_text_features = blip_text_encoder(negated_captions, max_length=77, device=device).float()

                # Compute the logits and loss
                with torch.cuda.amp.autocast():
                    logits, _, _ = combiner(reference_features, text_features, target_features)
                    ground_truth = torch.arange(images_in_batch, dtype=torch.long, device=device)
                    loss_f = crossentropy_criterion(logits, ground_truth)

                    Tij = target_features.detach().unsqueeze(1).repeat(1, images_in_batch, 1).transpose(0,1)
                    ti = negated_text_features.unsqueeze(1).repeat(1, images_in_batch, 1)
                    # below is just the combiner.forward() with some modifications
                    # operations done to Ri should be before the dimension changes
                    reference_image_features = combiner.project_targets(reference_features)
                    reference_image_features = torch.nn.functional.normalize(reference_image_features, dim=-1)
                    Ri = reference_image_features.unsqueeze(1).repeat(1, images_in_batch, 1).unsqueeze(-1)
                    Pi = combiner.combine_features(Tij, ti).unsqueeze(-2)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--blip-model-path", type=str, help="Path to the fine-tuned BLIP model")
    parser.add_argument("--blip-bs", default=32, type=int, help="Batch size during BLIP feature extraction")
    # image preprocessing
    ...
    if args.api_key and args.workspace:
        ...
        experiment = Experiment(
            ...
            project_name=f"BLIP4Cir_Bi combiner_training {args.dataset}",
