...
class FashionIQDataset(Dataset):
    ...
    The dataset manages an arbitrary numbers of FashionIQ category, e.g. only dress, dress+toptee+shirt, dress+shirt...
    """
...
class CIRRDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if use_sub:
            if self.split == 'train':
                print(f"Training with sub_split of fifth. Overwriting self.triplets")
                _ = input("\nConfirm!")
                with open(base_path / 'cirr_dataset' / 'cirr' / 'captions_sub' / f'cap.rc2.{split}_fifth.json') as f:
                    self.triplets = json.load(f)
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                rel_caption = self.__prepend_text__(raw_rel_caption, forward=True)
                rel_caption_negate = self.__prepend_text__(raw_rel_caption, forward=False)
                ...
                if self.split == 'train':
                    ...
                    return reference_image, target_image, rel_caption, rel_caption_negate
 ...
 * Copyright (c) 2024
 * All rights reserved.
 * SPDX-License-Identifier: Creative Commons BY-NC 4.0
 * The below code is a modified version of the code available at:
 * https://github.com/miccunifi/CIRCO/blob/main/src/dataset.py
 * For full license text of the original code, see the LICENSE file in the root of the CIRCO repo.
"""
